"""
strategy.py — Winston v7 — 10-Vote Statistical Engine

10 independent indicators each cast one vote: LONG or SHORT.
The side with more votes wins. Minimum votes required set in config.

The 10 Votes:
  ── Trend ──────────────────────────────────────────────────
  1.  EMA Trend         — EMA9 above/below EMA21
  2.  Higher Timeframe  — 15-min EMA9 above/below 15-min EMA21
                          (are we trading WITH the bigger trend?)

  ── Momentum ───────────────────────────────────────────────
  3.  MACD Cross        — MACD line above/below signal line
  4.  MACD Histogram    — histogram growing (momentum building) or shrinking
  5.  RSI Direction     — RSI rising or falling this bar
  6.  RSI Level         — RSI below 55 (room to go up) or above (room to fall)

  ── Price Structure ────────────────────────────────────────
  7.  VWAP Position     — price above/below day's volume-weighted avg price
  8.  Bollinger Position — price in lower half of bands (buy zone) or upper half

  ── Volume & Candles ───────────────────────────────────────
  9.  Volume Trend      — volume increasing on up bars (real buying) or down bars
  10. Candle Pattern    — current bar is bullish (closed near high) or bearish
                          (closed near low), filters out indecision doji bars

Scoring:
  - 6+ votes needed to enter (configurable via MIN_VOTE_SCORE in config.py)
  - Tie or below threshold = HOLD, no trade
  - Discord shows full vote breakdown on every trade
"""
...
def _resample_to_15min(df: pd.DataFrame) -> pd.DataFrame:
    """
    Converts 5-min bars to 15-min bars for higher timeframe trend check.
    Groups every 3 rows into one 15-min bar.
    """
    if len(df) < 3:
        return df
    df15 = df.copy()
...
def get_vote_score(df: pd.DataFrame, ticker: str = "") -> dict:
    """
    Returns full vote breakdown:
    {
      "long_votes":  int (0-10),
      "short_votes": int (0-10),
      "signal":      "LONG" | "SHORT" | "HOLD",
      "votes":       { vote_name: "LONG"|"SHORT" },
      "info":        { indicator values for logging },
      "atr":         float
    }
    """
    df = compute_indicators(df)

    # Need enough bars for all indicators (MACD needs 26+, Bollinger needs 20+)
    if len(df) < 30:
    ...
    # Recent bar closes for momentum
    ...
    # ── Vote 1: EMA Trend ─────────────────────────────────────────────────────
    # Is the fast EMA above or below the slow EMA?
    votes["ema_trend"] = "LONG" if ema_fast > ema_slow else "SHORT"

    # ── Vote 2: Higher Timeframe (15-min) ─────────────────────────────────────
    # Resample to 15-min bars and check EMA trend on that timeframe.
    # Trading with the bigger trend wins more often.
    try:
        df15 = _resample_to_15min(df.reset_index(drop=True))
        if len(df15) >= 5:
            ...
            votes["higher_tf"] = votes["ema_trend"]  # fallback to 5-min
    except Exception:
    ...
    # Is the histogram growing (momentum building) or shrinking (fading)?
    votes["macd_histogram"] = "LONG" if macd_hist > prev_hist else "SHORT"

    # ── Vote 5: RSI Direction ─────────────────────────────────────────────────
    # Is RSI moving up (bullish momentum) or down (bearish)?
    votes["rsi_direction"] = "LONG" if rsi > prev_rsi else "SHORT"

    # ── Vote 6: RSI Level ─────────────────────────────────────────────────────
    # Below 55 = not overbought, still has room to move up
    # Above 55 = getting stretched, more likely to pull back
    votes["rsi_level"] = "LONG" if rsi < 55 else "SHORT"

    # ── Vote 7: VWAP Position ─────────────────────────────────────────────────
    # Above VWAP = buyers in control today
    # Below VWAP = sellers in control today
    votes["vwap_position"] = "LONG" if close > vwap else "SHORT"

    # ── Vote 8: Bollinger Band Position ───────────────────────────────────────
    # In the lower half of the bands = closer to support, likely to bounce up
    # In the upper half = closer to resistance, likely to pull back
    bb_midpoint = (bb_upper + bb_lower) / 2
    votes["bollinger"] = "LONG" if close < bb_midpoint else "SHORT"

    # ── Vote 9: Volume Trend ──────────────────────────────────────────────────
    # Are up bars happening on higher volume than down bars?
    # This detects REAL buying pressure vs weak moves
    ...
    # Is the current bar bullish (closed in upper 40% of its range)?
    # Or bearish (closed in lower 40%)?
    # Doji bars (tiny range) get neutral treatment — slight LONG bias for SPY
    bar_range = high - low
    if bar_range > 0:
        close_position = (close - low) / bar_range   # 0 = closed at low, 1 = at high
        if close_position >= 0.6:
            votes["candle"] = "LONG"     # strong bullish candle
        elif close_position <= 0.4:
            votes["candle"] = "SHORT"    # strong bearish candle
        else:
            votes["candle"] = "LONG"     # doji/indecision — slight LONG bias
    else:
        votes["candle"] = "LONG"         # flat bar, no signal
    ...
    log(f"[STRATEGY]{label} {signal} | "
        f"{long_votes}L/{short_votes}S votes | "
        f"close={close} rsi={rsi:.1f} macd={'▲' if macd > macd_sig else '▼'} "
        f"vwap={'above' if close > vwap else 'below'} "
        f"bb={'lower' if close < (bb_upper+bb_lower)/2 else 'upper'}")

    # Log individual votes for transparency
    vote_str = " | ".join(f"{k}={'🟢' if v == 'LONG' else '🔴'}" for k, v in votes.items())
...
def get_signal(df: pd.DataFrame, ticker: str = "") -> tuple:
    """Backwards-compatible wrapper — returns (signal, atr, info)."""
