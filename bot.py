...
def _process_ticker(ticker: str):
    ...
    # Long entry
    if signal == "LONG" and score >= config.SENTIMENT_BUY_MIN:
        ...
        try:
            ...
            notify_buy(ticker, "LONG", config.MAX_TRADE_DOLLARS, current_price, stop, tp, score, reason)
        except Exception as e:
            notify_error(f"Long order failed {ticker}: {e}")

    # Short entry
    elif signal == "SHORT" and score <= config.SENTIMENT_SHORT_MAX and _shorting_enabled:
        ...
        try:
            ...
            notify_buy(ticker, "SHORT", config.MAX_TRADE_DOLLARS, current_price, stop, tp, score, reason)
        except Exception as e:
            notify_error(f"Short order failed {ticker}: {e}")

    elif signal == "SHORT" and not _shorting_enabled:
        log(f"[BOT] {ticker} SHORT signal but shorting not enabled on this account — skipping.")
...
def run():
    ...
    # Check if account has shorting enabled
    _shorting_enabled = broker.is_account_shorting_enabled()
    log(f"[BOT] Shorting enabled: {_shorting_enabled}")

    notify("Bot started — SPY & QQQ, $2/trade, long + short. Watching every 60s.")
    ...
    while True:
        try:
            ...
            # Send daily summary once after close
