...
Polymarket-style 5-minute markets with a 10-vote statistical engine.

Every 5 minutes:
  1. Run all 10 indicators on SPY and QQQ
  2. Pick the ticker with the strongest vote majority
  3. Need 6+ votes to enter (configurable)
  4. If neither ticker hits 6 votes — skip, post "waiting for signal" to Discord
  5. Enter $5 position in winning direction
  6. Hold exactly 5 minutes (check every 30s for emergency stop)
  7. Close, post full result + vote breakdown to Discord
  8. Immediately start next cycle

Discord shows on every trade:
  🟢 LONG SPY — $5.00 @ ~$653.16
  Votes: 7L/3S | EMA🟢 HTF🟢 MACD🟢 MACDhist🟢 RSIdir🔴 RSIlvl🟢 VWAP🟢 BB🔴 VOL🟢 Candle🟢
  ✅ CLOSED LONG SPY (5MIN_CLOSE) | P&L: +$0.0234

Times (MST):
  Start: 6:45 AM MST (9:45 ET) — skip first 15 min of open
...
def _votes_to_emoji(votes: dict) -> str:
    """Convert vote dict to compact emoji string for Discord."""
    labels = {
        "ema_trend":      "EMA",
        "higher_tf":      "HTF",
        "macd_cross":     "MACD",
...
def _notify_entry(ticker: str, direction: str, price: float,
    ...
    vote_bar   = _votes_to_emoji(votes)
    vote_score = f"{long_votes}L/{short_votes}S"

    msg = (
        f"{emoji} **{action} {ticker}** — ${config.MAX_TRADE_DOLLARS:.2f} @ ~${price:.2f}\n"
        f"Votes: {vote_score} | {vote_bar}"
    )
    notify(msg)
...
def _open_position(ticker: str, direction: str, current_price: float,
    ...
    try:
        ...
        database.save_position(
            ...
            round(current_price * (1 - config.EMERGENCY_STOP_PCT), 4),
        ...
        log(f"[BOT] ✅ Opened {direction} {ticker} @ {current_price:.4f} "
            f"| {long_votes}L/{short_votes}S votes | holding 5 min")
        return True
...
def _check_emergency_stop(ticker: str) -> bool:
    if ticker not in _positions:
        return False
    try:
    ...
    move_against = (
        (entry - current_price) / entry if side == "LONG"
        else (current_price - entry) / entry
    )

    if move_against >= config.EMERGENCY_STOP_PCT:
        log(f"[BOT] ⚠️ EMERGENCY STOP — {ticker} moved {move_against:.2%} against {side}")
...
def _run_5min_cycle():
    ...
    # Score both tickers, pick the one with strongest vote majority
    best_ticker    = None
    best_direction = None
    best_margin    = -1
    best_long_v    = 0
    ...
    for ticker in _watchlist:
        ...
        if signal == "HOLD":
            log(f"[BOT] {ticker} — {result['long_votes']}L/{result['short_votes']}S "
                f"(need {config.MIN_VOTE_SCORE}) — not strong enough")
            continue
    ...
    if best_ticker is None:
        log("[BOT] ⏸️  No clear majority on either ticker — skipping cycle")
        notify(f"⏸️ Skipped — indicators split, waiting for clearer signal\n"
               f"Need {config.MIN_VOTE_SCORE}/10 votes to trade")
        return
    ...
    # Hold 5 minutes — check every 30s for emergency stop
    for _ in range(config.RUN_INTERVAL_SECS // 30):
        time.sleep(30)
        if best_ticker not in _positions:
            return
        _check_emergency_stop(best_ticker)
...
def run():
    ...
    notify(
        "Winston v7 🎯 10-Vote Polymarket Mode\n"
        "SPY & QQQ | $5/trade | Every 5 min\n"
        "6:45 AM MST → 12:50 PM MST\n"
        f"Needs {config.MIN_VOTE_SCORE}/10 votes to trade"
    )
    log("[BOT] Winston v7 started — 10-vote 5-min Polymarket loop")
